...
#   ./lab-preflight-and-smoke-test.sh              # defaults to --smoke-test
#
# Optional overrides:
...
case "${1:-}" in
  ...
  --install-only|--install)
    # --install is retained as a backward-compatible alias.
    mode="install-only"
    ;;
  -h|--help)
    echo "Usage:"
    echo "  $0 --install-only   Install and verify required Red Hat packages"
    echo "  $0 --smoke-test     Check prerequisites and test the running application"
    echo
    echo "Optional environment variables: API_BASE, FRONTEND_URL, LAB_PROJECT_DIR"
    exit 0
    ;;
  *)
...
if [[ "$mode" == "install-only" ]]; then
  heading "Installing Red Hat prerequisites"
  if ! command -v dnf >/dev/null 2>&1; then
    fail "dnf is unavailable; --install-only is intended for Red Hat-compatible systems"
    exit 1
  fi

  echo "Installing: python3 python3-pip unzip curl git"
  if sudo dnf install -y python3 python3-pip unzip curl git; then
    pass "Required Red Hat packages are installed"
  else
    fail "Package installation failed"
    echo "Ask the VM administrator to run:"
    echo "  sudo dnf install -y python3 python3-pip unzip curl git"
    exit 1
  fi
fi
...
if ((${#missing_packages[@]} > 0)); then
  ...
  echo "  sudo dnf install -y ${unique_packages[*]}"
  echo "Or run: $0 --install-only"
fi
...
if [[ "$mode" == "install-only" ]]; then
  ...
  if ((fail_count == 0)); then
    echo
    echo "${green}INSTALLATION READY: All required prerequisites are available.${reset}"
    echo "Next, start the backend and frontend, then run:"
    echo "  $0 --smoke-test"
    exit 0
  fi

  echo
  echo "${red}INSTALLATION INCOMPLETE: Resolve the failed checks above.${reset}"
  exit 1
fi
